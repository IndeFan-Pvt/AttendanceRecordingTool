...
def resolve_employee_row_index(worksheet, employee: EmployeeConfig, row_map: dict[str, int]) -> int | None:
    normalized_name = normalize_employee_name(employee.name)
    row_index = row_map.get(normalized_name)
        ...
        for day in range(1, days_in_month + 1):
            ...
            if row_index < worksheet.nrows and column_index < worksheet.ncols:
                ...
                if symbol in valid_symbols:
                    fixed[day] = symbol
        results[employee.name] = normalize_night_rest_assignments(fixed, shift_kinds, days_in_month)
    return results
        ...
        for day in range(1, days_in_month + 1):
            ...
            if row_index < worksheet.nrows and column_index < worksheet.ncols:
                ...
                if symbol == normalized_holiday_symbol:
                    holidays.append(day)
        results[employee.name] = tuple(holidays)
    return results
...
def read_previous_tail_from_workbook(
    ...
    for employee in employees:
        ...
        tail_by_employee[employee.name] = tuple(normalize_night_rest_sequence(tail, shift_kinds))
...
def resolve_reference_source(target_path: Path, fallback_source: Path) -> Path:
    ...
    candidate_names: list[str] = [fallback_source.name]
    if stem.endswith("_temp"):
        candidate_names.insert(0, target_path.with_name(stem.removesuffix("_temp") + target_path.suffix).name)
        ...
        try:
            for candidate_name in candidate_names:
                for candidate_path in search_root.rglob(candidate_name):
                    ...
                    if "old" in normalized_parts or "exe" in normalized_parts or "dist" in normalized_parts or "build" in normalized_parts:
                        continue
...
def resolve_previous_month_source(base_dir: Path, target_path: Path, reference_source: Path, year: int, month: int) -> Path | None:
    ...
    for original_name in (target_path.name, reference_source.name):
        replaced_name = re.sub(r"\d+月", f"{previous_month}月", original_name, count=1)
        if replaced_name == original_name:
            continue
        candidate_names.append(replaced_name)
        if replaced_name.endswith(f"_temp{target_path.suffix}"):
            candidate_names.append(replaced_name.replace(f"_temp{target_path.suffix}", target_path.suffix))
    ...
    recursive_matches = sorted(base_dir.rglob(f"*{previous_month}月*.xls"))
    for candidate_path in recursive_matches:
        normalized_parts = {part.lower() for part in candidate_path.parts}
        if "old" in normalized_parts:
            continue
    ...
    for employee_raw in raw["employees"]:
        ...
        employees.append(
            EmployeeConfig(
                ...
                min_rest_days=(None if employee_raw.get("min_rest_days") is None else int(employee_raw["min_rest_days"])),
                max_rest_days=(None if employee_raw.get("max_rest_days") is None else int(employee_raw["max_rest_days"])),
...
def build_schedule_model(config: SchedulerConfig) -> tuple[cp_model.CpModel, dict[tuple[str, int, str], cp_model.IntVar]]:
    ...
    for employee in config.employees:
        for day in range(config.days_in_month):
            ...
            for shift in shift_order:
                decision_vars[employee.name, day, shift] = model.NewBoolVar(f"shift_{employee.name}_{day}_{shift}")
                if shift not in employee.allowed_shifts:
                    model.Add(decision_vars[employee.name, day, shift] == 0)
                if effective_allowed_shifts is not None and shift not in effective_allowed_shifts:
                    model.Add(decision_vars[employee.name, day, shift] == 0)
            model.Add(sum(decision_vars[employee.name, day, shift] for shift in shift_order) == 1)

    for day in range(config.days_in_month):
        for unit, requirements in config.required_per_day.items():
            unit_names = [employee.name for employee in config.employees if employee.unit == unit]
            model.Add(sum(decision_vars[name, day, shift] for name in unit_names for shift in early_symbols) == requirements.get("early", 0))
            model.Add(sum(decision_vars[name, day, shift] for name in unit_names for shift in late_symbols) == requirements.get("late", 0))
        model.Add(sum(decision_vars[employee.name, day, shift] for employee in config.employees for shift in night_symbols) == config.night_total_per_day)
    ...
    for day, requirement in config.day_requirements.items():
        ...
        for shift, count in requirement.get("min", {}).items():
            model.Add(sum(decision_vars[employee.name, day_index, shift] for employee in config.employees) >= count)
        for shift, count in requirement.get("max", {}).items():
            model.Add(sum(decision_vars[employee.name, day_index, shift] for employee in config.employees) <= count)

    pair_vars: list[cp_model.IntVar] = []
    night_eligible_names: list[str] = []
    for employee in config.employees:
        name = employee.name
        if any(shift in night_symbols for shift in employee.allowed_shifts):
            night_eligible_names.append(name)

        if config.require_weekend_pair_rest and employee.require_weekend_pair_rest:
            for saturday_day, sunday_day in weekend_pairs:
                model.Add(
                    sum(decision_vars[name, saturday_day, shift] for shift in rest_like_symbols)
                    + sum(decision_vars[name, sunday_day, shift] for shift in rest_like_symbols)
                    >= 1
                )

        for day, shift in employee.fixed_assignments.items():
            if 1 <= day <= config.days_in_month:
                model.Add(decision_vars[name, day - 1, shift] == 1)

        for holiday_day in employee.specified_holidays:
            if 2 <= holiday_day <= config.days_in_month:
                model.Add(sum(decision_vars[name, holiday_day - 2, shift] for shift in night_symbols) == 0)

        tail = list(employee.previous_tail)
        if tail:
            last_shift = tail[-1]
            if last_shift in late_symbols:
                model.Add(sum(decision_vars[name, 0, shift] for shift in early_symbols) == 0)
            if last_shift in night_symbols:
                model.Add(sum(decision_vars[name, 0, shift] for shift in night_symbols + night_rest_symbols) == 1)

        if night_rest_symbols:
            previous_night = 1 if tail and tail[-1] in night_symbols else 0
            model.Add(sum(decision_vars[name, 0, shift] for shift in night_rest_symbols) <= previous_night)
            for day in range(1, config.days_in_month):
                model.Add(
                    sum(decision_vars[name, day, shift] for shift in night_rest_symbols)
                    <= sum(decision_vars[name, day - 1, shift] for shift in night_symbols)
                )

        for day in range(config.days_in_month - 1):
            model.Add(
                sum(decision_vars[name, day, shift] for shift in late_symbols)
                + sum(decision_vars[name, day + 1, shift] for shift in early_symbols)
                <= 1
            )
            model.Add(
                sum(decision_vars[name, day, shift] for shift in night_symbols)
                <= sum(decision_vars[name, day + 1, shift] for shift in night_symbols + night_rest_symbols)
            )

        if config.prefer_double_night:
            for day in range(config.days_in_month - 1):
                pair_var = model.NewBoolVar(f"night_pair_{name}_{day}")
                night_today = sum(decision_vars[name, day, shift] for shift in night_symbols)
                night_tomorrow = sum(decision_vars[name, day + 1, shift] for shift in night_symbols)
        ...
        work_flags = [1 if shift in work_symbols else 0 for shift in tail[-config.max_consecutive_work :]]
        work_flags.extend(sum(decision_vars[name, day, shift] for shift in work_symbols) for day in range(config.days_in_month))
        add_window_constraint(model, work_flags, config.max_consecutive_work + 1, config.max_consecutive_work)
        ...
        regular_rest_flags.extend(
            sum(decision_vars[name, day, shift] for shift in regular_rest_limit_symbols)
            for day in range(config.days_in_month)
        )
        ...
        all_rest_flags.extend(sum(decision_vars[name, day, shift] for shift in rest_like_symbols) for day in range(config.days_in_month))
        ...
        night_flags.extend(sum(decision_vars[name, day, shift] for shift in night_symbols) for day in range(config.days_in_month))
        ...
        for shift, minimum in employee.min_counts.items():
            model.Add(sum(decision_vars[name, day, shift] for day in range(config.days_in_month)) >= minimum)
        for shift, maximum in employee.max_counts.items():
            model.Add(sum(decision_vars[name, day, shift] for day in range(config.days_in_month)) <= maximum)

        if employee.min_rest_days is not None:
            model.Add(sum(decision_vars[name, day, shift] for day in range(config.days_in_month) for shift in rest_like_symbols) >= employee.min_rest_days)
        if employee.max_rest_days is not None:
            model.Add(sum(decision_vars[name, day, shift] for day in range(config.days_in_month) for shift in rest_like_symbols) <= employee.max_rest_days)
    ...
    if weekend_days and config.fairness_weekend_spread is not None:
        weekend_eligible = [employee.name for employee in config.employees if employee.employment == "full"]
        if weekend_eligible:
            weekend_counts = {
                name: sum(decision_vars[name, day, shift] for day in weekend_days for shift in rest_like_symbols)
                for name in weekend_eligible
            }
            ...
            for name in weekend_eligible:
                model.Add(weekend_counts[name] <= max_weekend)
                model.Add(weekend_counts[name] >= min_weekend)
            model.Add(max_weekend - min_weekend <= config.fairness_weekend_spread)
            objective_terms.append((max_weekend - min_weekend) * 100)

    if config.fairness_night_spread is not None and night_eligible_names:
        night_counts = {
            name: sum(decision_vars[name, day, shift] for day in range(config.days_in_month) for shift in night_symbols)
            for name in night_eligible_names
        }
        max_night = model.NewIntVar(0, config.days_in_month, "max_night")
        min_night = model.NewIntVar(0, config.days_in_month, "min_night")
        for name in night_eligible_names:
            model.Add(night_counts[name] <= max_night)
            model.Add(night_counts[name] >= min_night)
    ...
    for unit in {employee.unit for employee in config.employees}:
        unit_names = [employee.name for employee in config.employees if employee.unit == unit and employee.employment == "full"]
        if not unit_names:
            continue
        for label, symbols in (("early", early_symbols), ("late", late_symbols)):
            ...
            for name in unit_names:
                count = sum(decision_vars[name, day, shift] for day in range(config.days_in_month) for shift in symbols)
...
def build_schedule_from_solver(
    ...
    for employee in config.employees:
        ...
        for day in range(config.days_in_month):
            selected_shift = next(
                shift for shift in shift_order if solver.Value(decision_vars[employee.name, day, shift]) == 1
            )
            shifts.append(selected_shift)
        schedule[employee.name] = shifts
    return schedule
...
def build_relaxed_schedule_model(config: SchedulerConfig) -> tuple[cp_model.CpModel, dict[tuple[str, int, str], cp_model.IntVar]]:
    ...
    for employee in config.employees:
        for day in range(config.days_in_month):
            effective_allowed_shifts = effective_allowed_shifts_for_day(config, employee, day + 1)
            for shift in shift_order:
                decision_vars[employee.name, day, shift] = model.NewBoolVar(f"relaxed_shift_{employee.name}_{day}_{shift}")
                if shift not in employee.allowed_shifts:
                    model.Add(decision_vars[employee.name, day, shift] == 0)
                if effective_allowed_shifts is not None and shift not in effective_allowed_shifts:
                    model.Add(decision_vars[employee.name, day, shift] == 0)
    ...
    for employee in config.employees:
        ...
        for holiday_day in employee.specified_holidays:
            if 2 <= holiday_day <= config.days_in_month:
                previous_night = sum(decision_vars[name, holiday_day - 2, shift] for shift in night_symbols)
        ...
        if tail:
            last_shift = tail[-1]
            if last_shift in late_symbols:
                objective_terms.append(sum(decision_vars[name, 0, shift] for shift in early_symbols) * 600)
            if last_shift in night_symbols:
                objective_terms.append(
                    sum(decision_vars[name, 0, shift] for shift in shift_order if shift not in night_symbols + night_rest_symbols) * 900
                )

        for day in range(config.days_in_month - 1):
            late_today = sum(decision_vars[name, day, shift] for shift in late_symbols)
            early_tomorrow = sum(decision_vars[name, day + 1, shift] for shift in early_symbols)
            late_early_violation = model.NewBoolVar(f"relaxed_late_early_{name}_{day}")
            ...
            night_today = sum(decision_vars[name, day, shift] for shift in night_symbols)
            invalid_follow = sum(
                decision_vars[name, day + 1, shift]
                for shift in shift_order
                if shift not in night_symbols + night_rest_symbols
            )
            night_follow_violation = model.NewBoolVar(f"relaxed_night_follow_{name}_{day}")
            ...
            if night_rest_symbols:
                invalid_night_rest = sum(decision_vars[name, day + 1, shift] for shift in night_rest_symbols)
                previous_not_night = sum(
                    decision_vars[name, day, shift] for shift in shift_order if shift not in night_symbols
                )
                night_rest_violation = model.NewBoolVar(f"relaxed_night_rest_{name}_{day}")
        ...
        if config.require_weekend_pair_rest and employee.require_weekend_pair_rest:
            for saturday_day, sunday_day in weekend_pairs:
                saturday_work = sum(decision_vars[name, saturday_day, shift] for shift in work_symbols)
                sunday_work = sum(decision_vars[name, sunday_day, shift] for shift in work_symbols)
                weekend_violation = model.NewBoolVar(f"relaxed_weekend_pair_{name}_{saturday_day}")
        ...
        for shift, minimum in employee.min_counts.items():
            count = sum(decision_vars[name, day, shift] for day in range(config.days_in_month))
            deficit = model.NewIntVar(0, minimum, f"relaxed_min_{name}_{shift}")
            model.Add(count + deficit >= minimum)
            objective_terms.append(deficit * 120)
        for shift, maximum in employee.max_counts.items():
            count = sum(decision_vars[name, day, shift] for day in range(config.days_in_month))
            excess = model.NewIntVar(0, config.days_in_month, f"relaxed_max_{name}_{shift}")
            model.Add(count - excess <= maximum)
            objective_terms.append(excess * 120)

        if employee.min_rest_days is not None:
            rest_count = sum(decision_vars[name, day, shift] for day in range(config.days_in_month) for shift in rest_like_symbols)
            rest_deficit = model.NewIntVar(0, employee.min_rest_days, f"relaxed_min_rest_{name}")
            model.Add(rest_count + rest_deficit >= employee.min_rest_days)
            objective_terms.append(rest_deficit * 120)
        if employee.max_rest_days is not None:
            rest_count = sum(decision_vars[name, day, shift] for day in range(config.days_in_month) for shift in rest_like_symbols)
            rest_excess = model.NewIntVar(0, config.days_in_month, f"relaxed_max_rest_{name}")
    ...
    for day in range(config.days_in_month):
        for unit, requirements in config.required_per_day.items():
            ...
            for label, symbols in (("early", early_symbols), ("late", late_symbols)):
                ...
                actual = sum(decision_vars[name, day, shift] for name in unit_names for shift in symbols)
                deficit = model.NewIntVar(0, len(unit_names), f"relaxed_{unit}_{label}_deficit_{day}")
                excess = model.NewIntVar(0, len(unit_names), f"relaxed_{unit}_{label}_excess_{day}")
        ...
        night_actual = sum(decision_vars[employee.name, day, shift] for employee in config.employees for shift in night_symbols)
        ...
        for employee in config.employees:
            ...
            for day_offset in range(31):
                ...
                if should_write_employee_row(employee) and day_offset < config.days_in_month:
                    value = schedule[employee.name][day_offset]
                else:
                    value = ""
            ...
            for label, symbol in config.count_symbols.items():
                if should_write_employee_row(employee):
                    ...
                    count_value = sum(shift in counted_symbols for shift in schedule[employee.name])
                else:
                    count_value = ""
...
def validate_schedule(config: SchedulerConfig, schedule: dict[str, list[str]]) -> dict[str, object]:
    ...
    used_special_leave = sorted(
        employee.name
        for employee in config.employees
        if "特" in schedule[employee.name]
    )
    ...
    for day in range(config.days_in_month):
        for unit, requirements in config.required_per_day.items():
            unit_names = [employee.name for employee in config.employees if employee.unit == unit]
            early_count = sum(schedule[name][day] in early_symbols for name in unit_names)
            late_count = sum(schedule[name][day] in late_symbols for name in unit_names)
            ...
            if late_count != requirements.get("late", 0):
                ...
                issues.append(message)
                staffing_issues.append(message)
        night_count = sum(schedule[employee.name][day] in night_symbols for employee in config.employees)
    ...
    for employee in config.employees:
        shifts = list(employee.previous_tail) + schedule[employee.name]
        if config.require_weekend_pair_rest and employee.require_weekend_pair_rest:
            for saturday_day, sunday_day in weekend_pairs:
                saturday_shift = schedule[employee.name][saturday_day]
                sunday_shift = schedule[employee.name][sunday_day]
                if saturday_shift not in rest_like_symbols and sunday_shift not in rest_like_symbols:
                    message = f"{employee.name}: {saturday_day + 1}日(土)-{sunday_day + 1}日(日) の両方が勤務"
                    issues.append(message)
        ...
        for day in range(1, config.days_in_month + 1):
            ...
            if effective_allowed_shifts is None:
                continue
            shift = schedule[employee.name][day - 1]
            if shift not in effective_allowed_shifts:
                message = f"{employee.name}: {day}日の勤務 {shift} が曜日別勤務制限に違反"
                issues.append(message)
        ...
        for index, shift in enumerate(shifts):
            ...
            if work_streak > config.max_consecutive_work:
                message = f"{employee.name}: {config.max_consecutive_work}連勤超過"
                issues.append(message)
                work_streak_violations.append(message)
            if night_streak > config.max_consecutive_night:
                message = f"{employee.name}: 夜勤{config.max_consecutive_night}連続超過"
                issues.append(message)
                night_streak_violations.append(message)
            if regular_rest_streak > config.max_consecutive_rest:
                message = f"{employee.name}: 休{config.max_consecutive_rest}連続超過"
                issues.append(message)
                regular_rest_violations.append(message)
            if all_rest_streak > config.max_consecutive_rest_with_special:
                message = f"{employee.name}: 休/特{config.max_consecutive_rest_with_special}連続超過"
                issues.append(message)
            ...
            if shift in night_rest_symbols:
                if previous_shift in night_symbols:
                    ...
                    if night_rest_chain_streak == 10:
                        ...
                        message = f"{employee.name}: 夜勤明け休みが10回以上連続 ({day_number}日目で到達)"
        ...
        night_rest_chain_max[employee.name] = max_night_rest_chain_streak
            ...
            if holiday_day == 1:
                if employee.previous_tail:
                    if employee.previous_tail[-1] in night_symbols:
                        specified_holiday_night_violations.append(f"{employee.name}: 指定休日 {holiday_day} 日の前日が夜")
                else:
                    specified_holiday_unchecked.append(f"{employee.name}: 指定休日 {holiday_day} 日は前月末情報がないため未検証")
                continue
            if shifts[holiday_day - 2] in night_symbols:
                specified_holiday_night_violations.append(f"{employee.name}: 指定休日 {holiday_day} 日の前日が夜")

    night_eligible = [employee.name for employee in config.employees if any(config.shift_kinds.get(shift) == "night" for shift in employee.allowed_shifts)]
    night_counts = {name: sum(shift in night_symbols for shift in schedule[name]) for name in night_eligible}
    day_shift_counts = {employee.name: sum(shift in day_symbols for shift in schedule[employee.name]) for employee in config.employees}
    standard_day_shift_counts = {
        employee.name: sum(shift in standard_day_shift_symbols for shift in schedule[employee.name])
        for employee in config.employees
    }
    standard_day_shift_target_counts = {
        employee.name: standard_day_shift_counts[employee.name]
        for employee in config.employees
        if int(employee.min_counts.get("日", 0)) > 0
    }
    standard_day_shift_violations = [
        f"{employee.name}: 通常の日が {standard_day_shift_counts[employee.name]} 回で、最低 {employee.min_counts['日']} 回に未達"
        for employee in config.employees
        if int(employee.min_counts.get("日", 0)) > standard_day_shift_counts[employee.name]
    ]
    double_night_counts = {
        employee.name: sum(
            1
            for day in range(config.days_in_month - 1)
            if schedule[employee.name][day] in night_symbols and schedule[employee.name][day + 1] in night_symbols
        )
        for employee in config.employees
        if any(config.shift_kinds.get(shift) == "night" for shift in employee.allowed_shifts)
    }
    weekend_rest = {
        employee.name: sum(schedule[employee.name][day] in rest_like_symbols for day in weekend_days)
        for employee in config.employees
        if employee.employment == "full"
    }
    return {
        "issues": issues,
...
def build_validation_results(config: SchedulerConfig, validation: dict[str, object]) -> list[dict[str, object]]:
    full_time_employees = [employee.name for employee in config.employees if employee.employment == "full"]
    special_leave_names = list(validation.get("used_special_leave", []))
    part_time_entries = [
        f"{employee.name}: {next((shift for shift in employee.allowed_shifts if shift in {'5.5', '6.0'}), '')}"
        for employee in config.employees
    ...
    for employee in config.employees:
        ...
        if non_holiday_fixed_assignments:
            fixed_assignment_entries.append(
                f"{employee.name}: {', '.join(f'{day}日={shift}' for day, shift in non_holiday_fixed_assignments[:5])}"
            )
        if specified_holiday_days:
            displayed_days = ", ".join(f"{day}日" for day in specified_holiday_days[:5])
            if len(specified_holiday_days) > 5:
                displayed_days += ", ..."
            specified_holiday_entries.append(f"{employee.name}: {displayed_days}")
    ...
    results.extend([
        ...
        },
        {
            ...
            "detail": "5連勤超過はありません。" if not validation["work_streak_violations"] else "5連勤超過があります。",
        ...
        },
        {
            "category": "ルール適合性",
            "title": "夜勤回数の平等化",
            "status": "適合" if config.fairness_night_spread is None or validation["night_spread"] <= config.fairness_night_spread else "不適合",
            "detail": f"夜勤回数のばらつきは {validation['night_spread']} 回です。",
            "evidence": [f"{name}: {count}回" for name, count in validation["night_counts"].items()],
        },
        {
            "category": "ルール適合性",
            "title": "夜夜の2連続を1回以上入れる配慮",
            "status": "適合" if double_night_total > 0 else "要注意",
            "detail": "夜夜の2連続が少なくとも1回あります。" if double_night_total > 0 else "夜夜の2連続は確認できませんでした。",
            "evidence": [f"{name}: {count}回" for name, count in validation.get("double_night_counts", {}).items()],
        },
        {
            "category": "ルール適合性",
            "title": "土日休みの平等化",
...
def with_generate_overrides(config: SchedulerConfig, args: argparse.Namespace) -> SchedulerConfig:
    ...
    if manual_year == year and manual_month == month:
        manual_fixed_assignments = read_fixed_assignments_from_workbook(
            reference_source,
            config.sheet_index,
            config.employees,
            config.shift_kinds,
    ...
    for employee in config.employees:
        fixed_assignments = dict(manual_fixed_assignments.get(employee.name, {}))
        fixed_assignments.update(employee.fixed_assignments)
        specified_holidays = tuple(sorted({*employee.specified_holidays, *target_specified_holidays.get(employee.name, ())}))
        for holiday_day in target_specified_holidays.get(employee.name, ()):
